...
exports.submitOwnerSettlement = async (req, res) => {
    const { unit_id, amount, due_date, slip_image, submitted_amount, submitted_date } = req.body;

    try {
        console.log(`📥 Received Owner Settlement request for Unit ID: ${unit_id}`);
        await db.query(
            'INSERT INTO payments (unit_id, amount, balance, due_date, status, slip_image, submitted_amount, submitted_date) VALUES (?, ?, ?, ?, "under_review", ?, ?, ?)',
            [unit_id, amount, amount, due_date, slip_image, submitted_amount, submitted_date]
        );
        try {
            const [admins] = await db.query('SELECT id FROM users WHERE role != "resident" LIMIT 1');
            if (admins.length > 0) {
                const adminId = admins[0].id;
                const notifMessage = `A Property Settlement of LKR ${Number(amount).toLocaleString()} has been uploaded and is waiting for your verification.`;

                await db.query(
                    'INSERT INTO notifications (user_id, title, message, type) VALUES (?, "New Property Settlement", ?, "payment")',
                    [adminId, notifMessage]
                );
            }
        } catch (notifErr) {
            console.log("⚠️ Admin notification failed:", notifErr.message);
        }

        res.status(201).json({ message: 'Owner settlement submitted successfully!' });
    } catch (error) {
        console.error('❌ Owner Settlement Error:', error);
        res.status(500).json({ message: 'Failed to submit owner settlement', error: error.message });
    }
};
...
cron.schedule('0 0 1 * *', async () => {
    console.log('⏳ Running Automated Billing Engine (Rent + Maintenance)...');
    try {
        const query = `
            SELECT 
            ...
            FROM users usr
            JOIN units un ON usr.unit_no = un.unit_number
            LEFT JOIN rooms r ON un.category = r.title
            WHERE usr.role = 'resident' AND usr.unit_no IS NOT NULL
        `;
        ...
        for (const res of residents) {
            ...
            if (res.unit_status === 'owner_occupied') {

                billAmount = MAINTENANCE_FEE;
                notifTitle = "Monthly Maintenance Bill";
                notifMsg = `Your Monthly Maintenance Fee of LKR ${MAINTENANCE_FEE.toLocaleString()} is due. Due date: ${formattedDueDate}.`;
            } else {
                billAmount = (Number(res.rent_amount) || 0) + MAINTENANCE_FEE;
                notifTitle = "Monthly Rent & Maintenance Bill";
                notifMsg = `Your Monthly Rent + Maintenance bill of LKR ${billAmount.toLocaleString()} is auto-generated. Due date: ${formattedDueDate}.`;
            }

            await db.query(
                'INSERT INTO payments (unit_id, amount, balance, due_date, status) VALUES (?, ?, ?, ?, "pending")',
                [res.unit_id, billAmount, billAmount, formattedDueDate]
            );

            try {
                await db.query(
                    'INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, "maintenance")',
                    [res.resident_id, notifTitle, notifMsg]
                );
            } catch (err) {}
        }
        console.log('🎉 Automated Billing Completed for both Owners & Tenants!');
    } catch (error) {
