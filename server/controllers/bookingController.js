...
exports.createBooking = async (req, res) => {
    const { resident_id, facility_name, booking_date, start_time, end_time, status } = req.body;
    ...
    try {
        ...
        `;
        const [duplicateBookings] = await db.query(duplicateBookingQuery, [
            resident_id, facility_name, booking_date, end_time, start_time
        ]);

        if (duplicateBookings.length > 0) {
            return res.status(400).json({
                message: 'You have already booked this facility for the selected time slot. Multiple bookings for the same time are not allowed.'
            });
        }
        ...
        const initialStatus = status || (facility.requires_approval ? 'pending' : 'confirmed');
...
exports.updateBooking = async (req, res) => {
    const { id } = req.params;
    const { facility_name, booking_date, start_time, end_time, status, rejection_reason } = req.body;
    ...
    try {
        ...
        await db.query(
            'UPDATE bookings SET facility_name = ?, booking_date = ?, start_time = ?, end_time = ?, status = ?, rejection_reason = ? WHERE id = ?',
            [facility_name, formattedDate, start_time, end_time, status, rejection_reason || null, id]
        );
