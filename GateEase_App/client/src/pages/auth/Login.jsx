...
import axios from 'axios'; // <-- අලුතින් Import කළා Backend එකට කතා කරන්න
...
const Login = () => {
    ...
    // වෙනස් කළ handleSubmit Function එක
    const handleSubmit = async (e) => {
        e.preventDefault();

        try {
            // Backend API එකට Email සහ Password යවනවා
            const response = await axios.post('http://localhost:5000/api/auth/login', formData);

            // Backend එකෙන් එවන දත්ත (Token, Role, Name) අරගන්නවා
            const { token, role, name } = response.data;

            // ඒවා Browser එකේ සේව් කරගන්නවා (ඉස්සරහට ඕන වෙන නිසා)
            ...
            if (role === 'admin') {
                ...
                // වෙනත් Role එකක් ආවොත් (Security, Maintenance වගේ)
                navigate('/');
            }

        } catch (error) {
            // Email හරි Password හරි වැරදි නම් මේක පෙන්වනවා
