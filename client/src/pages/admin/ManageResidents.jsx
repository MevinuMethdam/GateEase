...
import { Users, Plus, Mail, Phone, Lock, User, Edit2, Trash2, X, Check, Shield, TrendingUp, UserMinus, AlertTriangle, CreditCard, PhoneCall, Car, Wrench, Calendar, UserCheck } from 'lucide-react';
...
const ManageResidents = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        nic: '', emergency_contact: '', vehicle_number: '', category: 'General', agreement_end_date: ''
    });
    ...
    const fetchData = async () => {
        try {
            const [resResidents, resUnits, resApproved] = await Promise.all([
            ...
            const dbResidents = resResidents.data || [];
            const dbUnits = resUnits.data || [];
            const dbBookings = resApproved.data || [];

            setResidents(dbResidents);
            setAllUnits(dbUnits);

            const lockedSet = new Set();
            const activeOccupantsMap = new Map();

            const userMap = new Map();
            dbResidents.forEach(u => {
                userMap.set(u.id, u);
                userMap.set(u.name, u);
            });

            dbBookings.forEach(booking => {
                const unitNo = (booking.assigned_unit || booking.unitNumber || '').toUpperCase().trim();
                if (!unitNo) return;
                const matchedUser = userMap.get(booking.user_id) || userMap.get(booking.residentName);
                if (matchedUser) {
                    if (matchedUser.status === 'inactive') {
                        lockedSet.add(unitNo);
                    } else {
                        activeOccupantsMap.set(unitNo, matchedUser);
                    }
                }
            });

            dbResidents.forEach(user => {
                if (user.unit_no) {
                    const unitNo = String(user.unit_no).toUpperCase().trim();
                    if (user.status === 'inactive') {
                        lockedSet.add(unitNo);
                    } else {
                        activeOccupantsMap.set(unitNo, user);
                    }
                }
            });

            const purelyVacantUnits = dbUnits.filter(unit => {
                const unitNoUpper = String(unit.unit_no || unit.unit_number || '').toUpperCase().trim();
                let computedStatus = unit.status ? String(unit.status).toLowerCase() : 'vacant';

                if (lockedSet.has(unitNoUpper)) {
                    computedStatus = 'locked';
                } else if (activeOccupantsMap.has(unitNoUpper)) {
                    computedStatus = 'occupied';
                }

                return computedStatus === 'vacant';
            });

            setUnits(purelyVacantUnits);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.role === 'resident') {
                dataToSend.occupancy_type = formData.status;
                dataToSend.occupancy_status = formData.status;
            }
            ...
            if (formData.role === 'maintenance') {
                ...
                dataToSend.agreement_end_date = null;
            } else {
                dataToSend.category = null;
            }

            await axios.post('http://localhost:5000/api/users/residents', dataToSend);
            if (formData.role === 'resident' && formData.unit_id) {
                const targetUnit = units.find(u => String(u.id) === String(formData.unit_id));
                if (targetUnit) {
                    try {
                        await axios.put(`http://localhost:5000/api/units/${targetUnit.id}`, {
                            unit_number: targetUnit.unit_number || targetUnit.unit_no,
                            floor_number: targetUnit.floor_number,
                            square_feet: targetUnit.square_feet,
                            status: formData.status,
                            category: targetUnit.category
                        });
                    } catch (syncError) {
                        console.error("Failed to sync unit status:", syncError);
                    }
                }
            }
            ...
            setFormData({ name: '', email: '', password: '', phone: '', unit_id: '', status: 'owner_occupied', role: 'resident', nic: '', emergency_contact: '', vehicle_number: '', category: 'General', agreement_end_date: '' });
    ...
    const handleActivate = async (id, name) => {
        if (window.confirm(`Are you sure you want to reactivate the account for ${name}?`)) {
            try {
                await axios.put(`http://localhost:5000/api/users/${id}/activate`);
                fetchData();
                alert(`✅ Account for ${name} has been successfully reactivated.`);
            } catch (error) {
                    ...
                    {chartData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip contentStyle={{ backgroundColor: '#111111', borderColor: 'rgba(255,255,255,0.05)', borderRadius: '16px', color: '#fff', fontSize: '13px', boxShadow: '0 15px 40px rgba(0,0,0,0.8)' }} itemStyle={{ fontWeight: 'bold' }} cursor={{ stroke: 'rgba(255,255,255,0.1)', strokeWidth: 1, strokeDasharray: '5 5' }} />
                        ...
                        {formData.role === 'resident' && (
                            <div className="pt-2 pb-2 space-y-4 animate-in fade-in duration-300">
                                ...
                                {formData.status === 'owner_occupied' && (
                                    <div className="relative group/input animate-in fade-in duration-300">
                ...
                <div className="lg:col-span-2 bg-[#0A0A0A]/90 backdrop-blur-2xl p-8 rounded-[32px] border border-white/[0.05] shadow-[0_15px_40px_rgba(0,0,0,0.5)]">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                            {residents.map((res) => {
                                const assignedUnit = allUnits.find(u =>
                                    (u.unit_no && res.unit_no && String(u.unit_no).toUpperCase().trim() === String(res.unit_no).toUpperCase().trim()) ||
                                    (u.unit_number && res.unit_no && String(u.unit_number).toUpperCase().trim() === String(res.unit_no).toUpperCase().trim())
                                );

                                const isOwnerUser =
                                    (assignedUnit && assignedUnit.status && String(assignedUnit.status).toLowerCase().includes('owner')) ||
                                    [res.status, res.occupancy_status, res.role, res.occupancy_type].some(val => val && String(val).toLowerCase().trim().includes('owner'));

                                const isTenantUser =
                                    !isOwnerUser && (
                                        (assignedUnit && assignedUnit.status && String(assignedUnit.status).toLowerCase().includes('rent')) ||
                                        [res.status, res.occupancy_status, res.role, res.occupancy_type].some(val => val && String(val).toLowerCase().trim().includes('rent'))
                                    );
                                            ...
                                            <>
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="flex items-center gap-2 mt-1">
                                                                {res.role === 'maintenance' ? (
                                                                    <p className="text-[9px] text-blue-400 uppercase tracking-widest font-bold">
                                                                        MAINTENANCE {res.category ? `• ${res.category}` : ''}
                                                                    </p>
                                                                ) : (
                                                                    <p className="text-[9px] text-[#666666] uppercase tracking-widest font-bold">
                                                                        {isOwnerUser ? 'OWNER' : isTenantUser ? 'TENANT' : 'RESIDENT'} {res.unit_no && <span className="ml-2 text-[#848484]">Unit: {res.unit_no}</span>}
                                                                    </p>
                                                                )}
                                                                {res.status === 'inactive' && <span className="text-[8px] bg-red-500/20 text-red-500 px-1.5 py-0.5 rounded border border-red-500/30 uppercase font-bold tracking-wider ml-2">Deactivated</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="py-5 pr-4 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        {res.role === 'resident' && !isOwnerUser && res.status !== 'inactive' && (
                                                        ...
                                                        {res.role === 'resident' && !isOwnerUser && res.status === 'inactive' && (
                                                            <button onClick={() => handleActivate(res.id, res.name)} className="w-8 h-8 rounded-full bg-[#111111] border border-white/[0.05] flex items-center justify-center text-[#848484] hover:text-green-500 hover:border-green-500/30 hover:bg-green-500/5 transition-all" title="Reactivate Account"><UserCheck className="w-3.5 h-3.5" /></button>
                                                        )}
