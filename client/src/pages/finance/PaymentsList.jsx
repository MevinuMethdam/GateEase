...
const PaymentsList = () => {
    ...
    const [formData, setFormData] = useState({ unit_id: '', amount: '', due_date: '', resident_id: '', resident_name: '' });
    ...
    const fetchData = async () => {
        try {
            const [resPayments, resUnits, resUsers, resApproved] = await Promise.all([
            ...
            const enhancedPayments = dbPayments.map(pay => {
                let rName = pay.resident_name;

                if (!rName || rName.trim() === '' || rName === 'N/A') {
                    const unitNo = String(pay.unit_number || pay.unit_no || '').toUpperCase().trim();
                    if (activeOccupantsMap.has(unitNo)) {
                        const occupant = activeOccupantsMap.get(unitNo);
                        rName = occupant.name || occupant.residentName || occupant.full_name;
                    }
                }

                return { ...pay, resident_name: rName || 'N/A' };
            });

            setPayments(enhancedPayments);

            const billableUnits = dbUnits.filter(unit => {
                const unitNoUpper = String(unit.unit_no || unit.unit_number || '').toUpperCase().trim();
                let residentData = null;

                if (!lockedSet.has(unitNoUpper) && activeOccupantsMap.has(unitNoUpper)) {
                    const occupant = activeOccupantsMap.get(unitNoUpper);

                    const isTenant = [
                        occupant.status,
                        occupant.occupancy_status,
                        occupant.role,
                        occupant.occupancy_type,
                        unit.status
                    ].some(val => val && String(val).toLowerCase().trim().includes('rent'));

                    if (isTenant) {
                        residentData = occupant;
                        unit.current_resident_id = residentData.id || residentData.user_id;
                        unit.current_resident_name = residentData.name || residentData.residentName || residentData.full_name || '';
                        unit.computedStatus = 'rented';
                        return true;
                    }
                }
                return false;
    ...
    const handleGenerateInvoice = async (e) => {
        ...
        const existingInvoice = payments.find(pay => {
            if (pay.unit_number === selectedUnit.unit_number || pay.unit_number === selectedUnit.unit_no) {
                const payDate = new Date(pay.due_date || pay.created_at);
        ...
        if (existingInvoice) {
            alert(`⚠️ An invoice for Unit ${selectedUnit.unit_number || selectedUnit.unit_no} has already been generated for this month (${selectedDate.toLocaleString('default', { month: 'long', year: 'numeric' })}).\n\nYou cannot generate duplicate bills for the same month.`);
            return;
        }
        ...
        try {
            ...
            setFormData({ unit_id: '', amount: '', due_date: '', resident_id: '', resident_name: '' });
            fetchData();
        } catch (error) {
                                ...
                                onChange={(e) => {
                                    ...
                                    if (selectedUnit) {
                                        if (selectedUnit.category) {
                                            const baseRent = ROOM_PRICES[selectedUnit.category] || 0;
                                            autoAmount = baseRent > 0 ? baseRent + 5000 : '';
                                        }
                                        resId = selectedUnit.current_resident_id || '';
                                        resName = selectedUnit.current_resident_name || '';
                                    }
                            ...
                            >
                                ...
                                {units.map(u => (
                                    <option key={u.id} value={u.id}>
                                        {u.unit_number || u.unit_no} {u.category ? `- ${u.category}` : ''}
                                    </option>
                                ))}
                            </select>
                        </div>
