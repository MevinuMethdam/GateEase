...
const ManageBookings = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post('http://localhost:5000/api/bookings', { ...formData, status: 'pending' });
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        ...
        if (newStatus === 'rejected') {
            rejectionReason = window.prompt("Please enter the reason for rejecting this booking:");
            if (rejectionReason === null) return;
            if (rejectionReason.trim() === "") return alert("A reason is required to reject a booking.");
        } else {
    ...
    const handleUpdate = async () => {
        try {
            await axios.put(`http://localhost:5000/api/bookings/${editingBooking.id}`, {
                ...
                rejection_reason: editingBooking.rejection_reason
            });
                                ...
                                <tr key={bk.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                                    {editingBooking && editingBooking.id === bk.id ? (
                                        <>
                                            ...
                                            <td className="py-2">
                                                <select value={editingBooking.status} onChange={(e) => {

                                                    if(e.target.value === 'rejected') {
                                                        const rsn = window.prompt("Enter rejection reason:");
                                                        if(rsn) setEditingBooking({...editingBooking, status: e.target.value, rejection_reason: rsn});
                                                    } else {
                                                        setEditingBooking({...editingBooking, status: e.target.value, rejection_reason: null});
                                                    }
                                                ...
                                                {editingBooking.status === 'rejected' && (
                                                    <input type="text" placeholder="Reason..." value={editingBooking.rejection_reason || ''} onChange={(e) => setEditingBooking({...editingBooking, rejection_reason: e.target.value})} className="bg-dark-input text-red-400 mt-1 px-2 py-1 rounded-lg border border-red-500/20 outline-none w-full text-[10px]" />
                                                )}
                                            </td>
                                        ...
                                        <>
                                            ...
                                            <td className="py-4">
                                                ...
                                                {bk.status === 'rejected' && bk.rejection_reason && (
                                                    <p className="text-[9px] text-red-400/80 mt-1 italic break-words max-w-[120px]">
                                                        "{bk.rejection_reason}"
                                                    </p>
                                                )}
                                            </td>
