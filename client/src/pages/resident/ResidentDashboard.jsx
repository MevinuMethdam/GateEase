...
import {
    Wallet, AlertCircle, ShieldAlert, FileText, ArrowRight, Bell, Home, Key, Sparkles, TrendingUp, BarChart3, Receipt, XCircle
} from 'lucide-react';
...
const ResidentDashboard = () => {
    ...
    const fetchResidentData = async () => {
        ...
        try {
            let dashboardStats = { activeComplaints: 0, pendingVisitors: 0 };
            try {
                const response = await axios.get(`http://localhost:5000/api/dashboard/resident/${userId}`);
                dashboardStats = response.data;
            } catch (err) { console.warn('Using fallback for dashboard stats'); }
            ...
            myFilteredBills.forEach(bill => {
                if (bill.status !== 'paid') {
                    const balance = Number(bill.balance) || Number(bill.amount);
                    calculatedTotalDue += balance;
                }
            });

            myFilteredBills.sort((a, b) => new Date(b.created_at || b.due_date) - new Date(a.created_at || a.due_date));
            const recentTopBills = myFilteredBills.slice(0, 5);

            setStats({
                totalDue: calculatedTotalDue,
                activeComplaints: dashboardStats.activeComplaints || 0,
                pendingVisitors: dashboardStats.pendingVisitors || 0,
                recentBills: myFilteredBills
            });

        } catch (error) { console.error('Error fetching resident stats:', error); }
    };
    ...
    const fetchApprovedRooms = async () => {
        ...
        try {
            ...
            const activeReqs = reqRes.data.filter(r => r.status === 'Approved' || r.status === 'Rented' || r.status === 'Offer Made');
            ...
            const isUserInactive = me && me.status === 'inactive';

            let myActiveRoomsList = activeReqs.map(req => {
                const myRoom = allRooms.find(r => r.id === req.room_id);

                let finalDisplayStatus = req.status === 'Offer Made' ? 'PENDING SIGNATURE' : 'ACTIVE';
                if (isUserInactive) finalDisplayStatus = 'DEACTIVATED';

                return myRoom ? {
                    ...myRoom,
                    unitNumber: req.assigned_unit || "Pending Assignment",
                    reqId: req.id,
            ...
            if (me && me.unit_no) {
                const alreadyExists = myActiveRoomsList.some(r => r.unitNumber === me.unit_no);
                ...
                if (!alreadyExists) {
                    ...
                    let directDisplayStatus = me.status === 'owner_occupied' ? 'OWNER OCCUPIED' : 'ACTIVE';
                    if (isUserInactive) directDisplayStatus = 'DEACTIVATED';

                    if (myUnit) {
                        const myRoom = allRooms.find(r => r.title === myUnit.category);
                        if (myRoom) {
                            myActiveRoomsList.push({
                                ...myRoom,
                                unitNumber: me.unit_no,
                                reqId: 'direct-' + me.unit_no,
                                displayStatus: directDisplayStatus
                            });
                        }
                    } else {
                        myActiveRoomsList.push({
            ...
            setApprovedRooms(myActiveRoomsList);
        } catch (error) { console.error("Error fetching approved rooms:", error); }
    };

    useEffect(() => {
        fetchUserStatus(); fetchResidentData(); fetchNotifications(); fetchApprovedRooms();
    ...
    const getExpenseChartData = () => {
        ...
        stats.recentBills.forEach(bill => {
            ...
            if (bill.status === 'paid') { dataMap[monthYear].Paid += amount; }
            else {
                const balance = bill.balance !== undefined ? Number(bill.balance) : amount;
                dataMap[monthYear].Pending += balance;
                if (bill.status === 'partially_paid') { dataMap[monthYear].Paid += (amount - balance); }
            }
        });
        return Object.values(dataMap).sort((a, b) => a.sortDate - b.sortDate).map(item => ({ name: item.name, Paid: item.Paid, Pending: item.Pending }));
    };
    ...
    return (
        <div className="pb-10 max-w-7xl mx-auto font-sans relative z-10">
            ...
            {userStatus === 'inactive' && (
                <div className="mb-6 bg-red-500/10 border border-red-500/30 p-6 rounded-[24px] flex items-start gap-4 animate-in fade-in duration-500 shadow-[0_0_30px_rgba(239,68,68,0.15)] backdrop-blur-md">
                    ...
                    <div>
                        <h2 className="text-xl font-extrabold text-red-500 tracking-tight mb-1">Account Deactivated</h2>
                        <p className="text-red-400/80 text-sm font-medium leading-relaxed">
                            Your resident account has been temporarily or permanently deactivated by the system administrator. You cannot request new rooms or use certain facilities. Please contact management for more information.
                        </p>
                    </div>
                </div>
            )}
                ...
                {approvedRooms.length > 0 ? (
                    approvedRooms.map((room, index) => (
                        <div key={room.reqId || index} className="bg-[#0A0A0A] rounded-[24px] p-8 border border-white/[0.04] shadow-lg flex flex-col relative overflow-hidden group hover:border-[#C0DE1B]/30 transition-all duration-500 h-[320px]">
                            ...
                            <div className={`absolute top-0 right-0 w-[250px] h-[250px] rounded-full blur-[50px] pointer-events-none transition-colors duration-700 ${room.displayStatus === 'DEACTIVATED' ? 'bg-gradient-to-bl from-red-500/[0.08] to-transparent group-hover:from-red-500/[0.12]' : 'bg-gradient-to-bl from-[#C0DE1B]/[0.08] to-transparent group-hover:from-[#C0DE1B]/[0.12]'}`}></div>

                            <div className="flex items-center gap-5 relative z-10 mb-auto">
                                <div className={`w-14 h-14 bg-[#111111] border rounded-[16px] flex items-center justify-center shadow-[inset_0_1px_2px_rgba(255,255,255,0.05)] transition-all duration-500 ${room.displayStatus === 'DEACTIVATED' ? 'border-red-500/20 group-hover:shadow-[0_0_15px_rgba(239,68,68,0.15)]' : 'border-[#C0DE1B]/20 group-hover:shadow-[0_0_15px_rgba(192,222,24,0.15)]'}`}>
                                    <Key className={`w-6 h-6 ${room.displayStatus === 'DEACTIVATED' ? 'text-red-500' : 'text-[#C0DE1B]'}`} />
                                </div>
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <Sparkles className={`w-3 h-3 ${room.displayStatus === 'DEACTIVATED' ? 'text-red-500' : 'text-[#C0DE1B]'}`} />
                            ...
                            <div className="flex items-center gap-4 relative z-10 mt-6">
                                <div className="bg-[#111111]/80 flex-1 px-5 py-4 rounded-[16px] border border-white/[0.03] flex justify-between items-center group-hover:bg-[#111111] transition-colors">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${room.displayStatus === 'DEACTIVATED' ? 'bg-red-500 shadow-[0_0_5px_#EF4444]' : room.displayStatus === 'ACTIVE' || room.displayStatus === 'OWNER OCCUPIED' ? 'bg-[#C0DE1B] shadow-[0_0_5px_#C0DE1B]' : 'bg-orange-500 shadow-[0_0_5px_#F97316]'}`}></div>
                                        <p className={`${room.displayStatus === 'DEACTIVATED' ? 'text-red-500' : room.displayStatus === 'ACTIVE' || room.displayStatus === 'OWNER OCCUPIED' ? 'text-[#C0DE1B]' : 'text-orange-500'} text-[11px] md:text-xs font-extrabold tracking-wide`}>{room.displayStatus}</p>
                                    </div>
                                </div>

                                <div className="bg-[#111111]/80 flex-1 px-5 py-4 rounded-[16px] border border-white/[0.03] flex justify-between items-center group-hover:bg-[#111111] transition-colors">
                                    <p className="text-[#666666] text-[10px] font-bold uppercase tracking-widest">Unit</p>
                                    <p className="text-[#EAEAEA] text-sm font-extrabold tracking-tight">{room.displayStatus === 'DEACTIVATED' ? '-' : room.unitNumber}</p>
                                </div>
                            </div>
                        </div>
                    ))
                                ...
                                <div key={index} onClick={() => navigate('/resident/bills')} className="flex items-center justify-between p-4 rounded-[16px] bg-[#111111]/50 border border-white/[0.02] hover:bg-[#111111] hover:border-white/[0.05] transition-all duration-300 cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[11px] text-[#666666] font-medium leading-none">Due: <span className="text-[#848484]">{new Date(bill.due_date || bill.created_at).toLocaleDateString()}</span></p>
                                        </div>
                                    </div>
