...
import { User, Mail, Phone, Lock, ShieldCheck, Home, Save, CheckCircle, AlertTriangle, Edit3, X, Users, XCircle } from 'lucide-react';

const ResidentProfile = () => {
    const [profile, setProfile] = useState({ name: '', email: '', phone: '', unit_number: '', role: '', status: '' });
    ...
    const handleProfileUpdate = async (e) => {
        ...
        if (profile.status === 'inactive') {
            return alert("Your account is deactivated. You cannot update profile details.");
        }
        try {
    ...
    const handlePasswordUpdate = async (e) => {
        e.preventDefault();
        if (profile.status === 'inactive') {
            return alert("Your account is deactivated. You cannot change the password.");
        }
    ...
    const handleEmergencyContactUpdate = async (e) => {
        e.preventDefault();
        if (profile.status === 'inactive') {
            return alert("Your account is deactivated. You cannot update emergency contacts.");
        }
        try {
        ...
        <div className="pb-8 max-w-6xl mx-auto font-sans relative z-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 space-y-6">
                    <div className={`bg-[#0A0A0A]/90 backdrop-blur-2xl rounded-[32px] border p-8 text-center shadow-[0_15px_40px_rgba(0,0,0,0.5)] transition-all duration-500 relative overflow-hidden group ${profile.status === 'inactive' ? 'border-red-500/30 hover:border-red-500/50' : 'border-white/[0.05] hover:border-[#C0DE1B]/30'}`}>

                        <div className={`absolute top-0 right-0 w-[200px] h-[200px] rounded-full blur-[50px] pointer-events-none transition-colors duration-700 ${profile.status === 'inactive' ? 'bg-gradient-to-bl from-red-500/[0.08] to-transparent group-hover:from-red-500/[0.15]' : 'bg-gradient-to-bl from-[#C0DE1B]/[0.05] to-transparent group-hover:from-[#C0DE1B]/[0.1]'}`}></div>

                        <div className={`w-28 h-28 rounded-[24px] flex items-center justify-center text-[44px] font-extrabold mx-auto mb-5 border transition-all duration-500 ${profile.status === 'inactive' ? 'bg-red-500/10 text-red-500 border-red-500/30 shadow-[inset_0_2px_10px_rgba(239,68,68,0.1)] group-hover:shadow-[0_0_20px_rgba(239,68,68,0.15)]' : 'bg-[#111111] text-[#C0DE1B] border-[#C0DE1B]/20 shadow-[inset_0_2px_10px_rgba(192,222,24,0.1)] group-hover:shadow-[0_0_20px_rgba(192,222,24,0.15)]'}`}>
                        ...
                        <div className="mt-8 space-y-4 text-left border-t border-white/[0.05] pt-6 relative z-10">
                            <div className="flex items-center gap-4 bg-[#111111]/50 p-4 rounded-[20px] border border-white/[0.02]">
                                ...
                                <div>
                                    ...
                                    <p className="text-[14px] text-[#EAEAEA] font-extrabold">{profile.status === 'inactive' ? 'Unit Revoked' : profile.unit_number || 'Pending Assignment'}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 bg-[#111111]/50 p-4 rounded-[20px] border border-white/[0.02]">
                                <div className={`p-2.5 rounded-xl border ${profile.status === 'inactive' ? 'bg-red-500/10 border-red-500/20' : 'bg-green-500/10 border-green-500/20'}`}>
                                    {profile.status === 'inactive' ? <XCircle className="w-4 h-4 text-red-500" /> : <ShieldCheck className="w-4 h-4 text-green-500" />}
                                </div>
                                <div>
                                    <p className="text-[10px] text-[#666666] font-bold uppercase tracking-widest">Account Status</p>
                                    <p className={`text-[13px] font-extrabold tracking-wide ${profile.status === 'inactive' ? 'text-red-500' : 'text-green-500'}`}>
                                        {profile.status === 'inactive' ? 'Deactivated' : 'Active & Verified'}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-[#0A0A0A]/90 backdrop-blur-2xl rounded-[32px] border border-white/[0.05] p-8 shadow-[0_15px_40px_rgba(0,0,0,0.5)] group hover:border-red-500/20 transition-all duration-500 relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-5 relative z-10">
                            ...
                            <button
                                ...
                                className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${profile.status === 'inactive' ? 'bg-[#111111]/50 border-white/[0.02] text-[#444444] cursor-not-allowed' : 'bg-[#111111] border-white/[0.05] text-[#848484] hover:text-white hover:bg-white/[0.05]'}`}
                                title="Edit Contact"
                            >
                        ...
                        {emergencyContact.name ? (
                            ...
                            <div className={`text-center py-6 rounded-[20px] border border-dashed relative z-10 transition-colors ${profile.status === 'inactive' ? 'bg-[#111111]/10 border-white/[0.01] cursor-not-allowed' : 'bg-[#111111]/30 border-white/[0.02] cursor-pointer hover:bg-white/[0.02]'}`} onClick={() => profile.status !== 'inactive' && setIsEmergencyModalOpen(true)}>
                                ...
                                <p className="text-[#666666] text-[10px] mt-1">{profile.status === 'inactive' ? 'Account Deactivated' : 'Click to add emergency info'}</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="bg-[#0A0A0A]/90 backdrop-blur-2xl rounded-[32px] border border-white/[0.05] p-8 shadow-[0_15px_40px_rgba(0,0,0,0.5)]">
                        ...
                        <form onSubmit={handleProfileUpdate} className="space-y-6">
                            <div className="relative group/input">
                                ...
                                <input type="text" value={profile.name} onChange={(e) => setProfile({...profile, name: e.target.value})} disabled={profile.status === 'inactive'} className="w-full bg-[#111111] text-[#EAEAEA] pl-12 pr-4 py-4 rounded-[16px] border border-white/[0.05] focus:outline-none focus:border-[#C0DE1B]/50 focus:ring-1 focus:ring-[#C0DE1B]/50 transition-all shadow-inner font-semibold disabled:opacity-50" required />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="relative group/input">
                                    ...
                                    <input type="email" value={profile.email} onChange={(e) => setProfile({...profile, email: e.target.value})} disabled={profile.status === 'inactive'} className="w-full bg-[#111111] text-[#EAEAEA] pl-12 pr-4 py-4 rounded-[16px] border border-white/[0.05] focus:outline-none focus:border-[#C0DE1B]/50 focus:ring-1 focus:ring-[#C0DE1B]/50 transition-all shadow-inner font-semibold disabled:opacity-50" required />
                                </div>
                                <div className="relative group/input">
                                    ...
                                    <input type="text" value={profile.phone} onChange={(e) => setProfile({...profile, phone: e.target.value})} disabled={profile.status === 'inactive'} className="w-full bg-[#111111] text-[#EAEAEA] pl-12 pr-4 py-4 rounded-[16px] border border-white/[0.05] focus:outline-none focus:border-[#C0DE1B]/50 focus:ring-1 focus:ring-[#C0DE1B]/50 transition-all shadow-inner font-semibold disabled:opacity-50" required />
                                </div>
                            </div>

                            <div className="pt-4 text-right">
                                {profile.status === 'inactive' ? (
                                    <button type="button" disabled className="bg-red-500/10 border border-red-500/30 text-red-500 font-extrabold py-3.5 px-8 rounded-[16px] flex items-center gap-2 inline-flex cursor-not-allowed">
                                        <XCircle className="w-4 h-4" /> Account Deactivated
                                    </button>
                                ) : (
                        ...
                        <form onSubmit={handlePasswordUpdate} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="relative group/input">
                                    ...
                                    <input type="password" value={passwords.new} onChange={(e) => setPasswords({...passwords, new: e.target.value})} disabled={profile.status === 'inactive'} placeholder="••••••••" className="w-full bg-[#111111] text-[#EAEAEA] px-4 py-4 rounded-[16px] border border-white/[0.05] focus:outline-none focus:border-yellow-500/50 focus:ring-1 focus:ring-yellow-500/50 transition-all shadow-inner font-semibold placeholder:text-[#555555] disabled:opacity-50" required minLength="6" />
                                </div>
                                <div className="relative group/input">
                                    <label className="text-[10px] font-bold text-[#666666] uppercase tracking-widest mb-2 block">Confirm Password</label>
                                    <input type="password" value={passwords.confirm} onChange={(e) => setPasswords({...passwords, confirm: e.target.value})} disabled={profile.status === 'inactive'} placeholder="••••••••" className="w-full bg-[#111111] text-[#EAEAEA] px-4 py-4 rounded-[16px] border border-white/[0.05] focus:outline-none focus:border-yellow-500/50 focus:ring-1 focus:ring-yellow-500/50 transition-all shadow-inner font-semibold placeholder:text-[#555555] disabled:opacity-50" required minLength="6" />
                                </div>
                            </div>
                            <div className="pt-4 text-right">
                                {profile.status === 'inactive' ? (
                                    <button type="button" disabled className="bg-[#111111] border border-white/[0.05] text-[#555555] font-extrabold py-3.5 px-8 rounded-[16px] cursor-not-allowed">
                                        Update Password
                                    </button>
                                ) : (
