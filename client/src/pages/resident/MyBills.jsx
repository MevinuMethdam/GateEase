...
import { CreditCard, FileText, CheckCircle, AlertCircle, Upload, Clock, Calendar, DollarSign, FileUp, X, BarChart3, TrendingUp, Download, Printer, Home } from 'lucide-react';
...
const MyBills = () => {
    ...
    const [ownerSlipUrl, setOwnerSlipUrl] = useState('');
    const [ownerFileName, setOwnerFileName] = useState('');
    ...
    const fetchUserDataAndBills = async () => {
        ...
        try {
            const [userRes, allPaymentsRes, bookingsRes, unitsRes] = await Promise.all([
                axios.get('http://localhost:5000/api/users/residents').catch(() => ({ data: [] })),
                axios.get('http://localhost:5000/api/payments').catch(() => ({ data: [] })),
                axios.get('http://localhost:5000/api/rooms/approved').catch(() => ({ data: [] })),
                axios.get('http://localhost:5000/api/units').catch(() => ({ data: [] }))
            ]);

            const dbUsers = userRes.data || [];
            const dbPayments = allPaymentsRes.data || [];
            const dbBookings = bookingsRes.data || [];
            const dbUnits = unitsRes.data || [];

            const currentUser = dbUsers.find(u => String(u.id) === userId);

            const myUnitNumbers = new Set();
            const myUnitIds = new Set();

            if (currentUser) {
                if (currentUser.unit_no) myUnitNumbers.add(String(currentUser.unit_no).toUpperCase().trim());
                if (currentUser.unit_number) myUnitNumbers.add(String(currentUser.unit_number).toUpperCase().trim());
                if (currentUser.unit_id) myUnitIds.add(String(currentUser.unit_id));
            }

            dbBookings.forEach(booking => {
                const bUserId = String(booking.user_id || booking.resident_id || '');
                if (bUserId === userId) {
                    const assignedUnit = String(booking.assigned_unit || booking.unitNumber || booking.unit_no || '').toUpperCase().trim();
                    if (assignedUnit) myUnitNumbers.add(assignedUnit);
                    if (booking.unit_id) myUnitIds.add(String(booking.unit_id));
                }
            });

            dbUnits.forEach(unit => {
                const uResId = String(unit.resident_id || unit.user_id || unit.current_resident_id || '');
                if (uResId === userId) {
                    if (unit.unit_no) myUnitNumbers.add(String(unit.unit_no).toUpperCase().trim());
                    if (unit.unit_number) myUnitNumbers.add(String(unit.unit_number).toUpperCase().trim());
                    myUnitIds.add(String(unit.id));
                }
            });

            const myFilteredBills = dbPayments.filter(pay => {
                const payUnitNo = String(pay.unit_number || pay.unit_no || '').toUpperCase().trim();
                const payUnitId = String(pay.unit_id || '');
                const payResId = String(pay.resident_id || pay.user_id || pay.residentId || pay.userId || '');

                const isMyIdMatched = (payResId !== 'undefined' && payResId !== '' && payResId === userId);
                const isMyUnitNoMatched = (payUnitNo !== '' && myUnitNumbers.has(payUnitNo));
                const isMyUnitIdMatched = (payUnitId !== '' && myUnitIds.has(payUnitId));

                return isMyIdMatched || isMyUnitNoMatched || isMyUnitIdMatched;
            });

            myFilteredBills.sort((a, b) => new Date(b.created_at || b.due_date) - new Date(a.created_at || a.due_date));

            setBills(myFilteredBills);

            let isUserOwner = false;
            let currentUnitId = null;

            const myUnitInfo = dbUnits.find(u =>
                myUnitNumbers.has(String(u.unit_no || u.unit_number || u.unitNo || '').toUpperCase().trim()) ||
                myUnitIds.has(String(u.id))
            );

            if (myUnitInfo && myUnitInfo.status === 'owner_occupied') {
                isUserOwner = true;
                currentUnitId = myUnitInfo.id;
            } else if (currentUser && currentUser.status === 'owner_occupied') {
                isUserOwner = true;
                if (myUnitIds.size > 0) currentUnitId = Array.from(myUnitIds)[0];
            }
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => { setSlipUrl(reader.result); };
    ...
    const handleOwnerFileChange = (e) => {
        const file = e.target.files[0];
        if (file) {
            setOwnerFileName(file.name);
            const reader = new FileReader();
            reader.onloadend = () => { setOwnerSlipUrl(reader.result); };
            reader.readAsDataURL(file);
        }
    };
    ...
    const validateOwnerForm = () => {
        const errors = {};
        if (!ownerAmount || Number(ownerAmount) <= 0) errors.amount = "Please enter a valid settlement amount.";
        if (!ownerDate) errors.date = "Please select the date of payment.";
        if (!ownerSlipUrl) errors.file = "Proof of payment document is required.";
        setOwnerErrors(errors);
        return Object.keys(errors).length === 0;
    };

    const handleOwnerSettlementSubmit = async (e) => {
        e.preventDefault();
        if (!validateOwnerForm()) return;
        if (!userUnitId) {
            alert('❌ Cannot process payment: Unit ID not found. Please contact Admin.');
            return;
        }

        try {
            await axios.post(`http://localhost:5000/api/payments/owner-settlement`, {
                unit_id: userUnitId,
                amount: ownerAmount,
                due_date: ownerDate,
                slip_image: ownerSlipUrl,
                submitted_amount: ownerAmount,
                submitted_date: ownerDate,
                description: 'Property Purchase Settlement'
            });

            alert('✅ Property Settlement submitted successfully! Admin will verify and update Revenue.');
    ...
    return (
        <div className="relative z-10 font-sans print:bg-white print:text-black">
            <div className="print:hidden">
                ...
                {isOwner && (
                    <div className="bg-gradient-to-br from-purple-900/20 to-[#0A0A0A] backdrop-blur-2xl border border-purple-500/30 p-6 md:p-8 rounded-[32px] mb-8 shadow-[0_15px_40px_rgba(168,85,247,0.15)] relative overflow-hidden">
                        <div className="absolute -top-20 -right-20 w-[300px] h-[300px] bg-purple-500/10 rounded-full blur-[60px] pointer-events-none"></div>

                        <div className="flex items-center gap-4 mb-6 relative z-10">
                            <div className="w-12 h-12 bg-purple-500/10 border border-purple-500/30 rounded-[16px] flex items-center justify-center">
                                <Home className="w-6 h-6 text-purple-400" />
                            </div>
                            <div>
                                <h3 className="text-xl font-black text-white tracking-tight">Property Purchase Settlement</h3>
                                <p className="text-xs text-purple-300 font-bold uppercase tracking-widest mt-0.5">One-Time Ownership Payment</p>
                            </div>
                        </div>

                        <form onSubmit={handleOwnerSettlementSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">

                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Total Amount Paid (LKR)</label>
                                <div className="relative group">
                                    <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-purple-400" />
                                    <input
                                        type="number"
                                        placeholder="e.g. 25000000"
                                        value={ownerAmount}
                                        onChange={(e) => { setOwnerAmount(e.target.value); setOwnerErrors({...ownerErrors, amount: null}); }}
                                        className={`w-full bg-[#111] text-white pl-11 pr-4 py-3.5 rounded-xl border focus:outline-none transition-all shadow-inner font-bold ${ownerErrors.amount ? 'border-red-500/50' : 'border-purple-500/30 focus:border-purple-400'}`}
                                    />
                                </div>
                                {ownerErrors.amount && <p className="text-red-400 text-[10px] mt-1.5 font-bold flex items-center gap-1"><AlertCircle className="w-3 h-3"/> {ownerErrors.amount}</p>}
                            </div>

                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Transfer Date</label>
                                <div className="relative group">
                                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-purple-400" />
                                    <input
                                        type="date"
                                        value={ownerDate}
                                        onChange={(e) => { setOwnerDate(e.target.value); setOwnerErrors({...ownerErrors, date: null}); }}
                                        className={`w-full bg-[#111] text-white pl-11 pr-4 py-3.5 rounded-xl border focus:outline-none transition-all shadow-inner font-bold ${ownerErrors.date ? 'border-red-500/50' : 'border-purple-500/30 focus:border-purple-400'}`}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col justify-end">
                                {!ownerFileName ? (
                                    <div className="relative w-full h-[52px]">
                                        <input type="file" accept="image/*,application/pdf" onChange={(e) => { handleOwnerFileChange(e); setOwnerErrors({...ownerErrors, file: null}); }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                                        <div className={`w-full h-full rounded-xl border flex items-center justify-center gap-2 transition-all ${ownerErrors.file ? 'border-red-500/50 bg-red-500/10 text-red-400' : 'bg-purple-500/10 border-purple-500/30 text-purple-300 hover:bg-purple-500/20'}`}>
                                            <Upload className="w-4 h-4" /> <span className="text-sm font-bold">Attach Proof / Slip</span>
                                        </div>
                                        {ownerErrors.file && <p className="text-red-400 text-[10px] mt-1 absolute -bottom-5 font-bold">{ownerErrors.file}</p>}
                                    </div>
                                ) : (
                                    <div className="flex gap-2 items-center h-[52px]">
                                        <div className="flex-1 bg-purple-500/10 border border-purple-500/30 px-3 py-3 rounded-xl flex items-center gap-2 overflow-hidden text-purple-200">
                                            <CheckCircle className="w-4 h-4 shrink-0" />
                                            <span className="text-xs font-bold truncate">{ownerFileName}</span>
                                        </div>
                                        <button type="button" onClick={removeOwnerFile} className="w-[52px] h-[52px] bg-[#111] border border-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-red-500 hover:border-red-500/30 transition-all shrink-0">
                                            <X className="w-4 h-4" />
                                        </button>
                                    </div>
                                )}
                            ...
                            <div className="md:col-span-3">
                                <button type="submit" className="w-full bg-purple-500 hover:bg-purple-400 text-white font-black py-4 rounded-xl transition-all shadow-[0_0_15px_rgba(168,85,247,0.4)] hover:shadow-[0_0_25px_rgba(168,85,247,0.6)] flex justify-center items-center gap-2">
                                    Submit Settlement to Admin
                                </button>
                            </div>
                        </form>
                    </div>
                            ...
                            {bills.map((bill) => (
                                <tr key={bill.id} className="border-b border-white/[0.02] hover:bg-white/[0.02] transition-colors group">
                                    <td className="py-6 pl-4 flex items-center gap-5">
                                        ...
                                        <div>
                                            <p className="text-[#EAEAEA] font-extrabold text-[15px] tracking-tight">
                                                {bill.amount > 500000 ? "Property Purchase Settlement"
                                                    : bill.amount === 1000 ? "Monthly Maintenance Fee"
                                                        : "Apartment Rent & Maintenance"}
                                            </p>
                                            <p className="text-[12px] text-[#666666] font-medium mt-0.5">Unit: <span className="text-[#A0A0A0]">{bill.unit_number || bill.unit_no}</span></p>

                                            {(() => {
                                                const isOldBill = [250000, 185000, 120000, 85000].includes(Number(bill.amount));

                                                if (bill.amount > 1000 && bill.amount < 500000 && !isOldBill) {
                                                    return (
                                                        <div className="mt-2 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-md w-fit">
                                                            <p className="text-[10px] text-blue-400 font-extrabold tracking-wide">
                                                                Rent: LKR {(bill.amount - 1000).toLocaleString()} <span className="text-white mx-1">+</span> Maint: LKR 1,000
                                                            </p>
                                                        </div>
                                                    );
                                                }
                                    ...
                                    <td className="py-6 text-[#848484] font-medium text-sm">
                                        {new Date(bill.due_date || bill.created_at).toLocaleDateString()}
                                    </td>
                            ...
                            <div className="p-8 md:p-12 relative">
                                ...
                                <div className="space-y-5 mb-10">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-white font-bold print-text-black bg-white/5 px-3 py-1 rounded-lg print-bg-light">Unit {selectedReceipt.unit_number || selectedReceipt.unit_no}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
