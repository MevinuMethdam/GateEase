...
import ResidentAgreementGate from './components/ResidentAgreementGate';
...
const ResidentWrapper = () => {
    let residentId = localStorage.getItem('userId');
    ...
    if (!residentId) {
        ...
        if (token) {
            try {
                ...
                residentId = decodedData.id || decodedData._id;

            } catch (e) {
                console.error("Error decoding token to get resident ID:", e);
            }
        }
    }

    return (
        <ResidentAgreementGate residentId={residentId}>
            <ResidentLayout />
        </ResidentAgreementGate>
    );
};
    ...
    return (
        <Router>
            <Routes>
                ...
                <Route path="/security-staff" element={<SecurityDashboard />} />
